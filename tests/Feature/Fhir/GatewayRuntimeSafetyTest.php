...
class GatewayRuntimeSafetyTest extends TestCase
{
    ...
    public function test_gateway_agent_and_ingestion_runtime_post_routes_do_not_exist(): void
    {
        $forbidden = [
            ['POST', 'gateway'],
            ['POST', 'agent'],
        ...
        foreach ($forbidden as [$method, $uri]) {
            $this->assertFalse(
                ...
                "{$method} /{$uri} must not exist in Phase 9A."
            );
        }
    }
    ...
    public function test_phase_9a_does_not_call_fhir_write_methods_for_lesion_reads(): void
    {
        $this->actingAs((new User())->forceFill([
            'id' => 1,
            'name' => 'Gateway Runtime Safety User',
            'email' => 'gateway-runtime-safety@example.test',
        ]));
    ...
    public function test_frontend_read_only_route_guard_test_still_exists(): void
    {
